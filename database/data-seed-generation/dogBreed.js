//there are 100 records needed for breeds so the test so if making 300 every three will go up by 1 in their first breed
    //I will add two potential secondary breeds in this array to each dog (most will be multiple breeds) so they will randomly draw   from an array with numbers 1 - 200 where the last 50 spots are 0 so it is technically possible to get a pure-breed.
...
var dogBreed = function() {
  ...
  for (var k = 0; k < 100; k++) {
    dogbreeds.push([possibleBreeds[k], possibleBreeds[Math.floor(Math.random() * 200)], possibleBreeds[Math.floor(Math.random() * 200)]]);
    dogbreeds.push([possibleBreeds[k], possibleBreeds[Math.floor(Math.random() * 200)], possibleBreeds[Math.floor(Math.random() * 200)]]);
    dogbreeds.push([possibleBreeds[k], possibleBreeds[Math.floor(Math.random() * 200)], possibleBreeds[Math.floor(Math.random() * 200)]]);
  }
